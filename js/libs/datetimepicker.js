...
angular.module('ui.bootstrap.datetimepicker', [])
  ...
  .constant('dateTimePickerConfigValidation', function (configuration) {
    ...
    var validOptions = ['startView', 'minView', 'minuteStep', 'dropdownSelector', 'weekStart','view'];
  ...
  .directive('datetimepicker', ['dateTimePickerConfig', 'dateTimePickerConfigValidation','$filter', function (defaultConfig, validateConfigurationFunction,$filter) {
    "use strict";
          ...
          setTime: function (unixDate) {
            ...
            if (angular.isFunction(scope.onSetTime)) {
              ...
              switch(configuration.view){
                  case 'filter':
                      scope.ngModel = $filter('date')(newDate, 'mediumDate');
                      scope.$parent.applyFilter('date', scope.ngModel , 'Date selected');
                      break;
                  case 'forecastFilter':
                      scope.ngModel = $filter('date')(newDate, 'mediumDate');
                      scope.$parent.applyForecastFilter('date', scope.ngModel , 'Date selected');
                      break;
                  case 'task':
//                      scope.ngModel = $filter('date')(newDate, 'H:mm  -  MMM d, y');
//                      scope.ngModel = $filter('date')(newDate, 'dd MMM, yyyy  HH:mm');
                      break;
                  case 'chooseDate':default:
                      scope.ngModel = $filter('date')(newDate, 'd MMM, yyyy'); break;
                  case 'goToDate':
                      scope.ngModel = $filter('date')(newDate, 'd MMM, yyyy'); break;
                  case 'taskDetails':
                      scope.ngModel = $filter('date')(newDate, 'dd MMM,  HH:mm');
                      break;
              }
